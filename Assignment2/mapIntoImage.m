...
% Making sure the points selected are a convex
...
point = ones(3,numberOfPix);
for i=1:numberOfPix
    % calculating the source of each point inside the quadrilateral in imA
    vec = [insideIndX(i);
        insideIndY(i)
        1];
    point(:,i) = transfMat*vec;


    
end
    
    % Normalizing from homogenous coordinates 
    point = point./point(3,:);
    point(3,:) = []; %elimenating the Z coordinate
    
    % Applying the Bilinear interpolation from imB on the points
    % to calculate the value which we should assign to the pixel in
    % the new image
    V = interpolate(imageB,point);
    newImage(sub2ind(size(imageA),insideIndX,insideIndY)) = fix(V); 
    newImage = newImage';
    showImage(newImage);
